from spark_etl.environment import settings
...
def get_mongo_url(db_name: str) -> str:
    ...
    return f"mongodb://{creds['user']}:{creds['password']}@{host}:{creds['port']}/{db_name}?authSource=admin"

def get_psql_url(db_name: str) -> str:
    creds = get_db_creds(db_name)
    host = f"postgres-{db_name}-service"
    return f"jdbc:postgresql://{host}:{creds['port']}/{creds['db']}"
