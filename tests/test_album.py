...
class TestAlbumEndpoints:
    ...
    async def test_create_album_with_nonexistent_artist(
        ...
        admin_headers = await self._get_auth_headers(async_client)
        await self._upgrade_user(async_client, admin_headers)
        ...
        resp = await async_client.post("/album/", params=params, headers=admin_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN
        await self._delete_user(async_client, admin_headers)
