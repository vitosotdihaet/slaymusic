from dto.music import Track, NewTrack, TrackID, AlbumID, ArtistID, SearchParams
from repositories.interfaces import ITrackRepository
from repositories.helpers import RepositoryHelpers
from models.music import TrackModel, AlbumModel, ArtistModel
...
class SQLAlchemyTrackRepository(ITrackRepository, RepositoryHelpers):
    ...
    async def get_tracks_by_artist(
        ...
        async with self.session_factory() as session:
            ...
            query = select(TrackModel).where(TrackModel.artist_id == artist.id)
    ...
    async def get_tracks_by_album(
        ...
        async with self.session_factory() as session:
            ...
            query = select(TrackModel).where(TrackModel.album_id == album.id)
