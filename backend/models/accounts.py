...
class UserModel(AccountsModelBase):
    ...
    # liked_songs: Mapped[list["LikedSongModel"]] = relationship(
    #     "LikedSongModel", back_populates="user", cascade="all, delete-orphan"
    # )
...
# class LikedSongModel(AccountsModelBase):
#     __table_args__ = {'extend_existing': True}
...
#     user: Mapped["UserModel"] = relationship("UserModel", back_populates="liked_songs")

#     def __repr__(self):
#         return f"<LikedSong(user_id={self.user_id}, track_id={self.track_id})>"
...
#     playlist_id: Mapped[int] = mapped_column(ForeignKey("playlists.playlist_id", ondelete="CASCADE"), primary_key=True)
#     track_id: Mapped[int] = mapped_column(Integer, primary_key=True)

#     playlist: Mapped["PlaylistModel"] = relationship("PlaylistModel", back_populates="tracks")
